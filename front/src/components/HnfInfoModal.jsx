...
export default function HnfInfoModal({id}){
    ...
    const [clickList, setClickList] =useState({
      country:true, nutritional:false, allergy:false,
      pizza:true, side:false, kitchen:false        
    })
            ...
            <div className={'modal-container'} ref={modalBackground} onClick={e => {
              ...
              <div className={'modal-content'}>
               ...
               <ul className="info-list1">
                  <li><a><button className="country" id="btn1" onClick={(e)=>{colorControl(e.target);InfoChange("menu"); clickList.country=true; clickList.nutritional=false; clickList.allergy=false; }}>원산지</button></a></li>
                  <li><a><button id="btn2"  onClick={(e)=>{colorControl(e.target);InfoChange("nutritional");clickList.country=false; clickList.nutritional=true; clickList.allergy=false;  }}>영양성분</button></a></li>
                  <li><a><button id="btn3" onClick={(e)=>{colorControl(e.target); InfoChange("allergy"); clickList.country=false; clickList.nutritional=false; clickList.allergy=true; }}>알레르기 유발성분</button></a></li>
               </ul>
               <ul className="info-list2">
               <li><a><button className="line" id="btn4" onClick={(e)=>{borderControl(e.target); InfoChange("pizza");clickList.pizza=true; clickList.side=false; clickList.kitchen=false;}}>피자</button></a></li>
               <li><a><button id="btn5" onClick={(e)=>{borderControl(e.target); InfoChange("side"); clickList.pizza=false; clickList.side=true; clickList.kitchen=false;}}>사이드</button></a></li>
               <li><a><button id="btn6" onClick={(e)=>{borderControl(e.target);InfoChange("kitchen");clickList.pizza=false; clickList.side=false; clickList.kitchen=true;  }}>마이키친</button></a></li>
               </ul>
               <div>
                <table border='1'>
                <tr>
                ...
                <th className={`info-header ${clickList.nutritional && clickList.side  || clickList.nutritional && clickList.side || clickList.nutritional && clickList.kitchen? 'set-info-header':'info-header'}`}>{obj.name}</th>
                ...
                {infoContent.map(item=> (
                  <tr>
                  <td className={`${clickList.nutritional && clickList.pizza  ? 'set-info-contents':'info-contents'}`}>{item.name}</td>
                  <td className={`${clickList.allergy && clickList.kitchen ? 'set-info-contents2':'info-contents'}`}>{item.link}</td>
