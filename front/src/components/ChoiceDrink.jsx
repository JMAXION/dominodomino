    ...
    const increase = (bid) => {
        ...
        if (toppingFilter2.length > 0) {
            setDrinkQty(prevToppings => 
                prevToppings.map(t => 
                    t.bid === bid && t.quantity >= 6 ? { ...t, quantity: t.quantity - 1 } : t       
                )
            );
    ...
    return(
        <div className="step-wrap-side">
            ...
            <div id="topping1" className="tab-content-side">
                        ...
                        <div className="topping-name"><li>{obj.bname}<p>{obj.bprice}</p>
                            
                        </li>
                        </div>
                        <div className="qty-box">
                            <input type="text" className="topping-qty-txt" value={obj.quantity-1} ></input>
                                <button className="qty-minus" onClick={()=>{decrease(obj.bid)}} >-</button>
                                <button className="qty-plus" onClick={()=>{increase(obj.bid)}} >+</button>
