...
export default function Hnh({ depth2 }) {
  ...
  let today = new Date();
    ...
    const orderNumber = year+""+month+""+day+userInfo.userId + hour+""+minute
...
const [order, setOrder] = useState({
  ...
  function settingOrder(){
    ...
    for(let i=0; i<topping.length; i++){
      tpName += topping[i].tname + " " + topping[i].tprice+" "+topping[i].quantity + " "
    }
    for(let i=0; i<drinkMenu.length; i++){
      dringName += drinkMenu[i].bname + " " + drinkMenu[i].bprice+" "+drinkMenu[i].quantity + " "
    }
    
    let sideName= ''
    for(let i=0; i<sideMenu.length; i++){
      sideName += sideMenu[i].sname + " " + sideMenu[i].sprice+" "+sideMenu[i].quantity + " "
      setOrder({...order,   pizzaLeftName:pizzaLeft.pname,
...
useEffect(() => {
  ...
  const isInitialOrder = JSON.stringify(order) === JSON.stringify({
    pizzaLeftName: '',
    pizzaRightName: '',
    pizzaQty: 1,
    doughName: "없음",
  ...
  if (!isInitialOrder) {
    ...
    axios({
      ...
      console.error("There was an error with the order insert request!", error);
    });
  }
}, [order]);
          ...
          >
            ...
            <dl className="side">
              ...
              <dd>
                ...
                <ul className="select-menu">
                  ...
                  {sideMenu[1] && <li>{sideMenu[1].sname}(+{sideMenu[1].sprice.toLocaleString()}원)x{sideMenu[1].quantity}<em hidden>{tp+=sideMenu[1].sprice * sideMenu[1].quantity}</em></li>}
                  {sideMenu[2] && <li>{sideMenu[2].sname}(+{sideMenu[2].sprice.toLocaleString()}원)x{sideMenu[2].quantity}<em hidden>{tp+=sideMenu[2].sprice * sideMenu[2].quantity}</em></li>}
                  {sideMenu[3] && <li>{sideMenu[3].sname}(+{sideMenu[3].sprice.toLocaleString()}원)x{sideMenu[3].quantity}<em hidden>{tp+=sideMenu[3].sprice * sideMenu[3].quantity}</em></li>}
                  {sideMenu[4] && <li>{sideMenu[4].sname}(+{sideMenu[4].sprice.toLocaleString()}원)x{sideMenu[4].quantity}<em hidden>{tp+=sideMenu[4].sprice * sideMenu[4].quantity}</em></li>}
                  {sideMenu[5] && <li>{sideMenu[5].sname}(+{sideMenu[5].sprice.toLocaleString()}원)x{sideMenu[5].quantity}<em hidden>{tp+=sideMenu[5].sprice * sideMenu[5].quantity}</em></li>}                  
                </ul>
              </dd>
            </dl>
            <dl className="drink-etc">
              ...
              <dd>
                ...
                <input type="hidden" />
                <ul className="select-menu">
                  {drinkMenu[1] && <li>{drinkMenu[1].bname}(+{drinkMenu[1].bprice.toLocaleString()}원)x{drinkMenu[1].quantity} <em hidden>{tp+=drinkMenu[1].bprice * drinkMenu[1].quantity}</em></li>} 
                  {drinkMenu[2] && <li>{drinkMenu[2].bname}(+{drinkMenu[2].bprice.toLocaleString()}원)x{drinkMenu[2].quantity}<em hidden>{tp+=drinkMenu[2].bprice * drinkMenu[2].quantity}</em></li>}
                  {drinkMenu[3] && <li>{drinkMenu[3].bname}(+{drinkMenu[3].bprice.toLocaleString()}원)x{drinkMenu[3].quantity}<em hidden>{tp+=drinkMenu[3].bprice * drinkMenu[3].quantity}</em></li>}
                  {drinkMenu[4] && <li>{drinkMenu[4].bname}(+{drinkMenu[4].bprice.toLocaleString()}원)x{drinkMenu[4].quantity}<em hidden>{tp+=drinkMenu[4].bprice * drinkMenu[4].quantity}</em></li>}              
                  {drinkMenu[5] && <li>{drinkMenu[5].bname}(+{drinkMenu[5].bprice.toLocaleString()}원)x{drinkMenu[5].quantity}<em hidden>{tp+=drinkMenu[5].bprice * drinkMenu[5].quantity}</em></li>}
                </ul>
              </dd>
            </dl>
              <div className="total-price">
                  ...
                  (
                      ...
                      <div className="btn-wrap">
                        ...
                        <button type="button"  className="btn-type" >주문확인 </button>
                        </Link>
                      </div>
                    ...
                    <Link to="/login">
                      <div className="btn-wrap">
                    <button type="button"  className="btn-type">주문하기 </button>
                  </div>
                    </Link>
                  )
                  }
